...
Meta = {
	"Text": demojize(Config["Text"]["Content"]),
}

Meta["Image"] = Config["Media"]["Image"]["URL_or_Path"]

if os.path.exists(Config["Media"]["Image"]["URL_or_Path"]):
	Meta["Image"] = os.path.expanduser(Config["Media"]["Image"]["URL_or_Path"]).replace(os.environ["UserProfile"], "~")
...
if Config["Media"]["Video"]["Audio"]["URL_or_Path"]:
	...
	if os.path.exists(Config["Media"]["Video"]["Audio"]["URL_or_Path"]):
		Meta["Audio"] = os.path.expanduser(Config["Media"]["Video"]["Audio"]["URL_or_Path"]).replace(os.environ["UserProfile"], "~")
try:
...
try:
	...
	if not __Name.endswith(__Dynamic_Formats):
		...
		__Meta.__dict__ = {
			"chunks": [
				(b"tEXt", bytes("Text\x00{0}".format(Meta["Text"].encode("L1")), encoding = "U8")),
				(b"tEXt", bytes("Image\x00{0}".format(Meta["Image"].encode("L1")), encoding = "U8"))
			]
		}
		try:
			__Meta.__dict__["chunks"].append(
			(b"tEXt", bytes("Audio\x00{0}".format(Meta["Audio"]), encoding = "U8"))
		)
		except KeyError: pass
