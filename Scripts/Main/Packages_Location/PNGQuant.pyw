__PNGQuant = "PNGQuant"
__PNGQuant_Error_1 = "\n{0}PNGQuant{1} not found!".format(Styles.Warning, Styles.Reset)
		...
		for File in next(os.walk("."))[2]:
			...
			if __PNGQuant:
				if os.path.isfile(__PNGQuant):
					...
					print(__PNGQuant_Error_1 + ' {1}("{0}"){2}'.format(__PNGQuant, Styles.Meta_Info, Styles.Reset) + __PNGQuant_Error_2)
	else:
