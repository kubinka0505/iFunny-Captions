__FFmpeg = "FFmpeg"
__FFmpeg_Error_1 = "\n{0}FFmpeg{1} not found!".format(Styles.Warning, Styles.Reset)
		...
		for File in next(os.walk("."))[2]:
			...
			if __FFmpeg:
				if os.path.isfile(__FFmpeg):
					...
					raise SystemExit(__FFmpeg_Error_1 + ' {1}("{0}"){2}'.format(__FFmpeg, Styles.Meta_Info, Styles.Reset) + __FFmpeg_Error_2)
	else:
