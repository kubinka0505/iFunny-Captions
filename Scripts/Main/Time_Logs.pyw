...
print("Input {0} was converted to frames in:\t{2}{1}{3}.".format(
	"URL" \
		if Config["Media"]["Image"]["URL_or_Path"].startswith("http") \
		else "file", str(__CNV_TIME)[2:-3],
	Styles.Info, Styles.Reset
	)
)
print("Frames were processed in:\t\t{1}{0}{2}.".format(str(__MKE_TIME)[2:-3], Styles.Info, Styles.Reset))
print("{0} was made in:\t\t\t{2}{1}{3}.".format("PNG" if len(Frames) < 2 else "GIF", str(__SAV_TIME)[2:-3], Styles.Info, Styles.Reset))
