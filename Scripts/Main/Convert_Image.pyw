...
if __Folder_Name in next(os.walk("."))[1]:
	...
	try:
		...
		print("{1}WARNING{2}: User interrupted the frames conversion process. Converted {1}{0}{2} frames so far.{3}".format(str(len([File for File in next(os.walk("."))[2] if File.endswith("gif")])), Styles.Yellow, Styles.Reset, __BEL))
		pass
