...
- `requests`
- `textwrap`
- **`ffmpeg static executable binary >= 4.0.0`**
  - [Windows](https://www.videohelp.com/software/ffmpeg/old-versions "Windows: win") (Move `ffmpeg.exe` from `bin` to main folder)
...
- `Settings` :
  - `Speed / FPS` : **Default is 24**.
    - `Speed` : Single frame display time (in milliseconds). Applies to `PIL`.
    - `FPS` : Frames per seconds. Applies to `ffmpeg`.
  - `Saving_Method` : Image saving methots that *can* optimize its size.  **Default is `1`**.
    - `1` : `PIL`
    - `2` : `ffmpeg` **Does not support transparency**.
  - `Logs` : **Default is `false`**. Displays:
    - Image creation time
    - Input image details
    - Output image details
- `Text` : Image caption text.
- `Wrap` : Text-wrap, used to break the words. **Default is 14**.
2. Open `Run.sh`
