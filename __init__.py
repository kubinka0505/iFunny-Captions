...
__CNV_TIME = time()
os.system("{0} -i {1} Frame_%06d.{2} -hide_banner -loglevel panic".format(__FFMPEG, Get_Service(Config["Image"]["URL / Path"]), __FORMAT))
...
if not len(Frames) == 1:
	...
	#---#
	if Config["Settings"]["Optimize"]["Factor"] > 0:
		sleep(1)
		print("Optimizing with gifsicle...")
		os.system("gifsicle --careful -d={0} -b -O{1} --lossy={2} Images/{3} -w".format(Config["Settings"]["Delay"], Config["Settings"]["Optimize"]["Factor"], Config["Settings"]["Optimize"]["Lossy"], __NAME))
