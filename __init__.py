...
if Config()["Settings"]["Saving_Method"] == 1:
	...
	GIF.save("{0}{1}Captions{1}{2}_{3}.{4}".format(
		...
		duration = Config()["Settings"]["Speed / FPS"],
		)

if Config()["Settings"]["Saving_Method"] == 2:
	os.system("ffmpeg.exe -i Frame_%06d.png -vf palettegen palette.png -hide_banner -loglevel panic")
	os.system("ffmpeg.exe -i Frame_%06d.png -i palette.png -lavfi paletteuse Output.mp4 -hide_banner -loglevel panic")
	os.system("ffmpeg.exe -i Output.mp4 Captions{1}{2}_{3}.png -hide_banner -loglevel panic".format(Config()["Settings"]["Speed / FPS"], "\\" if system == "Windows" else "/", Config()["Text"].\
			replace(" ", "_").\
			replace("\\", "_").\
			replace("/", "_").\
			replace(":", "_").\
			...
			replace(">", "_").\
			replace("|", "_").\
			encode().decode("utf-8", errors = "strict")[0:192], Random_String(8)))
